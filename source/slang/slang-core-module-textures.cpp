        ...
        for (int includeMipInfo = 0; includeMipInfo < 2; ++includeMipInfo)
        {
                ...
                [&](StringBuilder& spirv, bool isRW, UnownedStringSlice imageVar)
            {
                ...
                if (isArray)
                {
                    ...
                    // For cube map arrays in SPIRV, we may need to adjust the array size
                    if (baseShape == SLANG_TEXTURE_CUBE)
                    {
                        spirv << "%uint_6 : $$uint = OpConstant $$uint 6; ";
                        spirv << "%_adjustedElements : $$uint = OpUDiv %_elements %uint_6; ";
                        convertAndStore(UnownedStringSlice("_adjustedElements"), "elements");
                    }
                }
